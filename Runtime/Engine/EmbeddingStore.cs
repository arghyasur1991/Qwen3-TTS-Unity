...
{
    /// <summary>
    /// Embedding matrices in AllocHGlobal: ~1.5 GB of npy tables plus the
    /// code-predictor projection tables, kept off the managed heap so they are
    /// neither scanned by the GC nor copied on access. Read from disk on
    /// construction (0.5-0.8 s) and freed on <see cref="Dispose"/>, which the
    /// engine calls when a checkpoint is evicted.
    /// </summary>
