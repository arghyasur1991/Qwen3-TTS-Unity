...
namespace QwenTTS.Editor
{
    ...
    /// what is currently resident. Read-only apart from Warm up, Evict and
    /// Unload everything: the package does not copy or download models, so
    ...
    internal sealed class QwenModelStatusWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if (!Engine.QwenModelPaths.RootIsExplicit)
            {
                EditorGUILayout.HelpBox(
                    ...
                    "Initialize to keep ~8 GB of weights per checkpoint out of your build.",
                    MessageType.Info);
            }
        ...
        void DrawCheckpoint(CheckpointStatus status, string blurb)
        {
            ...
            EditorGUILayout.LabelField("Resident", status.Loaded ? "yes (~7 GB)" : "no");
