...
- **The first and last three decoder layers**, which is the setting that makes
  ...
  and a Whisper transcription of the output drops a phoneme from the test
  sentence — WER 0.125 on a line that fp32 reads back exactly. Holding six of
...
def default_held_layers(model_path: str) -> list[int]:
    ...
    result drops a phoneme from the test sentence — WER 0.125. Holding
